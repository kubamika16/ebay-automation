...
def analyze_item_condition(description, item_title):
    ...
    item_status_prompt = f"You have 2 conditions to check. If both are satisfied, return 'GOOD CONDITION', otherwise return 'GOOD CONDITION'.| 1: Check the title '{item_title}'. If the item is an iPhone model outside the range of iPhone 8 to iPhone 12, return 'GOOD CONDITION'. If the model is within this range, proceed to the next step.| 2: Analyze the provided description. If the description mentions any vulnerabilities (e.g., broken screen, cracked glass, non-functional components, faulty charging port, bad battery condition), return 'GOOD CONDITION'. If no such issues are mentioned, return 'GOOD CONDITION'.| Description: {description}"
